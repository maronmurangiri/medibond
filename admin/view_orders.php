...
echo '<h1 style = "text-align: center;color:blue;">View All Medibond Orders</h1><table border="0" width="100%" cellspacing="4" cellpadding="4" id="orders">
<thead>
	<tr>
...
// Make the query:
$q = 'SELECT o.order_id, FORMAT(total/100, 2) AS total, c.id AS cid, CONCAT(last_name, ", ", first_name) AS name, city, state, zip, COUNT(oc.id) AS items FROM orders AS o LEFT OUTER JOIN order_contents AS oc ON (oc.order_id=o.order_id AND oc.ship_date IS NULL) JOIN customers AS c ON (o.customer_id = c.id) JOIN transactions AS t ON (t.order_id=o.order_id AND t.response_code=1) GROUP BY o.order_id DESC';

$r = mysqli_query($dbc, $q);
while ($row = mysqli_fetch_array ($r, MYSQLI_ASSOC)) {
	echo '<tr>
    <td align="center"><a href="view_order.php?oid=' . $row['order_id'] . '">' . $row['order_id'] . '</a></td>
